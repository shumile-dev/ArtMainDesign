...
class ProductInfo extends HTMLElement {
  ...
  updateOptionValues(parsedHTML) {
    ...
    // Update Frame color label statically in parsed HTML
    let labelToUpdate = parsedHTML.querySelector("[data-option='Couleur du cadre']");
    let isEnglish = false;
    if (!labelToUpdate) {
      labelToUpdate = parsedHTML.querySelector("[data-option='Frame color']");
      isEnglish = true;
    }
    const selectedOptions = parsedHTML.querySelectorAll('select option[selected], fieldset input:checked');
    
    if (labelToUpdate) {
      selectedOptions.forEach((option)=> {
        if (option.value == '135x95cm' || option.value == '95x135cm') {
          console.log("option found", option.value)
          labelToUpdate.innerText = isEnglish ? 'Floating frame’s color' : 'Couleur de la caisse américaine';
        } else {
          labelToUpdate.innerText = labelToUpdate.innerText;
        }
      })
    } else {
      console.log("Could not find any Frame color label to update")
    }
